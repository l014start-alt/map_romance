...
export default function RecordModal({ pin, onClose, onSubmit }: RecordModalProps) {
  ...
  if (done) {
    return (
      <div
        style={{ position: 'absolute', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'flex-end', background: 'rgba(0,0,0,0.40)', backdropFilter: 'blur(4px)' }}
      ...
      >
        <div style={{ width: '100%', background: '#FAF8F5', borderRadius: '14px 14px 0 0', minHeight: '40dvh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '48px 32px' }}>
          <p style={{ fontFamily: 'var(--font-sans)', fontSize: '15px', color: '#2A2520', lineHeight: 2.0, textAlign: 'center', wordBreak: 'keep-all' }}>
            기록이 접수되었습니다.
          </p>
          ...
          <button onClick={onClose} style={{ fontFamily: 'var(--font-sans)', fontSize: '11px', color: '#C0BEBB', letterSpacing: '0.06em', borderBottom: '1px solid #EDE9E4', paddingBottom: '1px', marginTop: '40px', cursor: 'pointer' }}>
            닫기
          </button>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div
        style={{ width: '100%', maxHeight: '90dvh', overflowY: 'auto', background: '#FAF8F5', borderRadius: '14px 14px 0 0', WebkitOverflowScrolling: 'touch', scrollbarWidth: 'none' }}
      >
        {/* 핸들 */}
        ...
        <form onSubmit={handleSubmit} style={{ padding: '28px 24px 48px', display: 'flex', flexDirection: 'column', gap: '36px' }}>
          ...
          {/* 주소 (LocationPicker에서 pre-fill, 직접 수정 가능) */}
          <div>
            <label style={{ display: 'block', fontFamily: 'var(--font-sans)', fontSize: '9px', color: '#C0BEBB', letterSpacing: '0.18em', marginBottom: '14px' }}>
